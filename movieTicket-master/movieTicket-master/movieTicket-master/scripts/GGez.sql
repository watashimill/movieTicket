...
CREATE TABLE IF NOT EXISTS `movieticket`.`Seats` (
  ...
  PRIMARY KEY (`Hall_Theatre_theatre_id`, `Hall_hall_id`),
...
CREATE TABLE IF NOT EXISTS `movieticket`.`Tickets` (
  ...
  INDEX `fk_Tickets_Seats1_idx` (`Seats_Hall_Theatre_theatre_id` ASC, `Seats_Hall_hall_id` ASC),
  ...
  CONSTRAINT `fk_Tickets_Seats1`
    FOREIGN KEY (`Seats_Hall_Theatre_theatre_id` , `Seats_Hall_hall_id`)
    REFERENCES `movieticket`.`Seats` (`Hall_Theatre_theatre_id` , `Hall_hall_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
